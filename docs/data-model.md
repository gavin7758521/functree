...
`view: "lite"` 可只返回轻量 ID 映射字段；`includeDetails` 可在查询 feature 时带出功能详情；`functree_resolve_stable_keys` 用于批量解析 stableKey；`functree_query_path_context` 用于按文件路径做增量扫描前查重；`functree_get_capability_matrix` 用于读取 canonical feature 的跨 map 状态矩阵；`functree_get_programming_context` 用于修改某个功能前读取优先级上下文；`functree_quality_report` 用于同步后检查覆盖缺口和低置信度问题。
